...
def main():
    ...
    # Downsample + interpolate missing values
    df_downsampled = numeric_cols.resample(DOWNSAMPLE_FREQ).mean()
    df_downsampled = df_downsampled.interpolate()
    ...
    valid = df_plot[['time_num', 'Gravity']].dropna()

    X = valid['time_num'].values.reshape(-1, 1)
    y = valid['Gravity'].values

    # Fit linear regression to gravity
    model = LinearRegression()
    model.fit(X, y)

    # Predict the time when gravity reaches target
    target_gravity = 1.017

    if abs(model.coef_[0]) < 1e-9:
        print("Gravity trend is flat; cannot estimate finish time.")
    ...
    ax1.axhline(target_gravity, color='green', linestyle='--',
                label=f'Target FG {target_gravity}')

    if time_to_target_sec is not None:
        ax1.axvline(time_to_target_sec, color='purple', linestyle='--',
